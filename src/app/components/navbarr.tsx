...
const ClientNavbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (isMenuOpen && navMenuRef.current && !navMenuRef.current.contains(e.target as Node) && hamburgerRef.current && !hamburgerRef.current.contains(e.target as Node)) {
        setMenuOpen(false);
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 shadow-lg">
      <div className="flex justify-around items-center mx-auto px-4 lg:px-8">
        ...
        >
          {NAV_LINKS.map((link) => (
            <li key={link.key} className="my-2 lg:my-0 lg:mx-4">
              <Link href={link.href} className="block text-center regular-16 text-green-900 cursor-pointer py-2 px-4 transition-colors hover:font-bold">
                {link.label}
              </Link>
            </li>
          ))}
        ...
        <button id="hamburger" name="hamburger" type="button" className="block lg:hidden" ref={hamburgerRef} aria-label="Toggle menu" onClick={handleClick}>
